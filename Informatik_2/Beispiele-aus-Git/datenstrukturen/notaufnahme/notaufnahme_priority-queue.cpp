 ...
 * In dieser zweiten Variante wird die Dringlichkeit beruecksichtigt
 * => Nutzung des Containers "priority_queue".
 * (Doku z.B. http://en.cppreference.com/w/cpp/container/priority_queue, letzter Zugriff: 7.11.2017)
 *
...
public:
    ...
    // Vergleichsoperator fuer zwei Patienten this < other
    // => Patient mit grosser Dringlichkeit (kleine Zahl) ist
    //    "groesser" als ein Patient mit kleiner Dringlichkeit (grosse Zahl)
...
{
    // groesserer Dringlichkeitswert bedeutet niedrigere Prioritaet => ist "kleiner"
    if (getDringlichkeit() > other.getDringlichkeit())
        return true;
    return false;
}
...
{
    char aktion = 'x';
    std::priority_queue<Patient> w; // "W"arteschlange
    ...
    std::cout << "Alice, Bob und Chris hinzugefuegt..." << std::endl;

    // -- einen Patienten behandeln
    if (!w.empty())
    {
        Patient p = w.top(); // Erstes Element kopieren
    ...
    std::cout << "Dennis, Emil, Franz, Gustav, Heinz hinzugefuegt..." << std::endl;
